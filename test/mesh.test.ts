...
describe('Mesh', () => {
  ...
  it('finds elements\' neighbourhoods', () => {
    const neighbourhoods: [number, ValueMeshElement[]][] =
  ...
  const expectElementHasNeighbours = (
    elementId: number,
    neighbourhoods: [number, ValueMeshElement[]][],
    expectedNeighbours: number[]
  ) => {
    const neighbours =
      neighbourhoods.find((n) => n[0] === elementId) || assert.fail()
    expect(neighbours[1].map((e) => e.id)).to.include.members(
      expectedNeighbours
    )
  }
})
